...
class TestBuildMemoPrompt:
    ...
    def test_contains_stock_memo_section_headers(self):
        prompt = self._prompt()
...
# ---------------------------------------------------------------------------
# market_snapshot_from_histories
# ---------------------------------------------------------------------------

class TestMarketSnapshotFromHistories:
    def test_returns_one_entry_per_index(self):
        histories = {sym: make_history([100.0, 102.0]) for sym in logic.MARKET_INDEXES.values()}
        snapshot = logic.market_snapshot_from_histories(histories)
        assert len(snapshot) == len(logic.MARKET_INDEXES)

    def test_labels_and_symbols_are_correct(self):
        histories = {sym: make_history([100.0, 102.0]) for sym in logic.MARKET_INDEXES.values()}
        snapshot = logic.market_snapshot_from_histories(histories)
        for entry, (label, symbol) in zip(snapshot, logic.MARKET_INDEXES.items()):
            assert entry["label"] == label
            assert entry["symbol"] == symbol

    def test_calculates_latest_close_and_change_pct(self):
        symbol = "^GSPC"
        histories = {symbol: make_history([100.0, 110.0])}
        # Patch MARKET_INDEXES so only one index runs
        original = logic.MARKET_INDEXES
        logic.MARKET_INDEXES = {"S&P 500": symbol}
        try:
            snapshot = logic.market_snapshot_from_histories(histories)
        finally:
            logic.MARKET_INDEXES = original

        assert snapshot[0]["close"] == pytest.approx(110.0)
        assert snapshot[0]["change_pct"] == pytest.approx(10.0)

    def test_negative_change_pct(self):
        symbol = "^GSPC"
        histories = {symbol: make_history([200.0, 190.0])}
        logic_indexes_backup = logic.MARKET_INDEXES
        logic.MARKET_INDEXES = {"S&P 500": symbol}
        try:
            snapshot = logic.market_snapshot_from_histories(histories)
        finally:
            logic.MARKET_INDEXES = logic_indexes_backup

        assert snapshot[0]["change_pct"] == pytest.approx(-5.0)

    def test_missing_symbol_returns_none_fields(self):
        snapshot = logic.market_snapshot_from_histories({})
        for entry in snapshot:
            assert entry["close"] is None
            assert entry["change_pct"] is None

    def test_none_history_returns_none_fields(self):
        histories = {sym: None for sym in logic.MARKET_INDEXES.values()}
        snapshot = logic.market_snapshot_from_histories(histories)
        for entry in snapshot:
            assert entry["close"] is None
            assert entry["change_pct"] is None

    def test_single_row_history_returns_none_fields(self):
        histories = {sym: make_history([100.0]) for sym in logic.MARKET_INDEXES.values()}
        snapshot = logic.market_snapshot_from_histories(histories)
        for entry in snapshot:
            assert entry["close"] is None
            assert entry["change_pct"] is None
    ...
    def test_empty_dataframe_returns_none_fields(self):
        histories = {sym: pd.DataFrame() for sym in logic.MARKET_INDEXES.values()}
        snapshot = logic.market_snapshot_from_histories(histories)
        for entry in snapshot:
            assert entry["close"] is None
            assert entry["change_pct"] is None
...
class TestBuildComparisonPrompt:
    def _prompt(self):
        return logic.build_comparison_prompt(
            company_name1="Apple Inc.",
            ticker1="AAPL",
            sector1="Technology",
            fundamentals_text1="Trailing P/E: 30.0",
            news_text1="Title: Apple news\nSummary: Details.",
            average_close1=190.0,
            company_name2="Microsoft Corporation",
            ticker2="MSFT",
            sector2="Technology",
            fundamentals_text2="Trailing P/E: 35.0",
            news_text2="Title: Microsoft news\nSummary: Other details.",
            average_close2=420.0,
            period="3mo",
        )

    def test_includes_both_company_names(self):
        prompt = self._prompt()
        assert "Apple Inc." in prompt
        assert "Microsoft Corporation" in prompt

    def test_includes_both_tickers(self):
        prompt = self._prompt()
        assert "AAPL" in prompt
        assert "MSFT" in prompt

    def test_uses_human_friendly_period_label(self):
        assert "the Last 3 Months" in self._prompt()

    def test_embeds_both_fundamentals(self):
        prompt = self._prompt()
        assert "Trailing P/E: 30.0" in prompt
        assert "Trailing P/E: 35.0" in prompt

    def test_embeds_both_news_blocks(self):
        prompt = self._prompt()
        assert "Apple news" in prompt
        assert "Microsoft news" in prompt

    def test_contains_all_comparison_section_headers(self):
        prompt = self._prompt()
        for header in [
    ...
    def test_includes_guardrails(self):
        prompt = self._prompt()
        assert "Do not recommend either company." in prompt
        assert "Never use dollar signs" in prompt

    def test_does_not_contain_single_stock_headers(self):
        prompt = self._prompt()
        assert "### Executive Summary" not in prompt
        assert "### Investment Thesis" not in prompt
        assert "### Bull Case" not in prompt

    def test_includes_both_companies_and_required_sections(self):
        prompt = logic.build_comparison_prompt(
            company_name1="Apple Inc.",
            ticker1="AAPL",
            sector1="Technology",
            fundamentals_text1="Trailing P/E: 30.0",
            news_text1="Title: Apple news\nSummary: Details.",
            average_close1=195.0,
            company_name2="Microsoft Corporation",
            ticker2="MSFT",
            sector2="Technology",
            fundamentals_text2="Trailing P/E: 28.0",
            news_text2="Title: Microsoft news\nSummary: Details.",
            average_close2=420.0,
            period="1mo",
        )
        ...
        assert "Ticker: AAPL" in prompt
        assert "Name: Microsoft Corporation" in prompt
        assert "Ticker: MSFT" in prompt
        assert "Average Closing Price During the Last Month: 195.0" in prompt
        assert "Average Closing Price During the Last Month: 420.0" in prompt
    ...
    def test_includes_no_recommendation_guardrail(self):
        prompt = logic.build_comparison_prompt(
            company_name1="Apple Inc.",
            ticker1="AAPL",
            sector1="Technology",
            fundamentals_text1="N/A",
            news_text1="",
            average_close1=195.0,
            company_name2="Microsoft Corporation",
            ticker2="MSFT",
            sector2="Technology",
            fundamentals_text2="N/A",
            news_text2="",
            average_close2=420.0,
            period="1mo",
        )

        assert "Do not tell the user to buy, sell, or hold either stock." in prompt


# ---------------------------------------------------------------------------
# market_snapshot_from_histories (integrated scenario)
# ---------------------------------------------------------------------------

class TestMarketSnapshot:
    def test_calculates_latest_close_and_daily_change(self):
        histories = {
            "^GSPC": make_history([100.0, 102.0]),
            "^IXIC": make_history([200.0, 190.0]),
            "^DJI":  make_history([300.0, 300.0]),
        }

        snapshot = logic.market_snapshot_from_histories(histories)

        assert snapshot[0]["label"] == "S&P 500"
        assert snapshot[0]["close"] == 102.0
        assert snapshot[0]["change_pct"] == pytest.approx(2.0)

        assert snapshot[1]["label"] == "Nasdaq"
        assert snapshot[1]["close"] == 190.0
        assert snapshot[1]["change_pct"] == pytest.approx(-5.0)

        assert snapshot[2]["label"] == "Dow Jones"
        assert snapshot[2]["close"] == 300.0
        assert snapshot[2]["change_pct"] == pytest.approx(0.0)

    def test_missing_or_insufficient_history_is_unavailable(self):
        histories = {
            "^GSPC": make_history([100.0]),
            "^IXIC": pd.DataFrame(),
            "^DJI":  None,
        }
        ...
        for index_data in snapshot:
            assert index_data["close"] is None
            assert index_data["change_pct"] is None
